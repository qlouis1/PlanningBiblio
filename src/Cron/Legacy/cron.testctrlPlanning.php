...
foreach ($sites as $site) {
    ...
    // on créé un tableau pour stocker les éléments par dates et sites
        $data[$site[0]][$date]=array("date"=>dateFr($date), "site"=>$site[1]);
        ...
        if (!$db->result) {
            $data[$site[0]][$date]["message"]="Le planning {$site[1]} du <strong>".dateFr($date)." <span style='color:red;'>n'est pas cr&eacute;&eacute;</span></strong>\n";
            continue;
        } else {
            ...
            if ($db->result) {
                $data[$site[0]][$date]["message"]="Le planning {$site[1]} du <strong>".dateFr($date)."</strong> est valid&eacute;\n";
            } else {
                $data[$site[0]][$date]["message"]="Le planning {$site[1]} du <strong>".dateFr($date)." <span style='color:red;'>n'est pas valid&eacute;</span></strong>\n";
            }
        }
      ...
      // On stock dans notre tableau data les éléments date, site, tableau
            $data[$site[0]][$date]['tableau'][$elem['nom']]["tableau"]=$elem['titre'];
            ...
            $data[$site[0]][$date]['tableau'][$elem['nom']]["data"]=$tab;
        }
    }
}
...
foreach ($sites as $s){
    ...
    $msg[$s[0]] .= "<ul>\n";
    foreach ($data[$s[0]] as $date) {
        $msg[$s[0]].="<li style='margin-bottom:15px;'>\n";
        if (array_key_exists("message", $date)) {
            $msg[$s[0]].=$date['message'];
        }
        if (array_key_exists("tableau", $date)) {
            $msg[$s[0]].="<br/>\nLes postes suivants ne sont pas occup&eacute;s :\n<ul>\n";
            foreach ($date['tableau'] as $tableau) {
                $msg[$s[0]].="<li>Tableau <strong>{$tableau['tableau']}</strong> :\n<ul>\n";
                foreach ($tableau['data'] as $poste) {
                    $msg[$s[0]].="<li>{$poste['poste']}, de ".heure2($poste['debut'])." &agrave; ".heure2($poste['fin'])."</li>\n";
                }
                $msg[$s[0]].="</ul>\n";
            }
            $msg[$s[0]].="</ul>\n";
