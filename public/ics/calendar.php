...
$db->selectInnerJoin(
    ...
    array("personnel","id"),
    array("date", "debut", "fin", "poste", 'site', 'absent', 'ur1_forced', 'supprime'),
    array(),
  array("perso_id"=>$id),
    $requete_personnel,
...
if (isset($planning)) {
    ...
    foreach ($planning as $elem) {
        ...
        if ($elem['absent'] == 1 && $elem['ur1_forced'] != "1") {
            continue;
        }
    ...
    foreach ($tab as $elem) {
        ...
        $ical[]="CLASS:PUBLIC";
        // UR1: 04B Export "non bloquant" events as free
        ...
        // UR1: 04C Custom attribute to filter back on import. Attribute is conserved if user copy/paste the event in Partage as long as mail client handles X-MICROSOFT tags
