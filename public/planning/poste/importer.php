    ...
    foreach ($dates as $elem) {
        ...
        if ($db->result) {
            foreach ($db->result as $elem2) {
                ...
                // UR1: 03A score off agents with an imported absence
                $db2 = new db();
                $db2->select("absences", "*", "`debut`<'$fin' AND `fin`>'$debut' AND `perso_id`='{$elem2['perso_id']}' AND `motif` LIKE '% Partage' $filter ");
